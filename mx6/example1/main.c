...

{
	...
	if (soc_version_check(soc_list) < 0) {
		fprintf(stderr, "The soc_version_check() function has failed: %s!\n", strerror(errno));
		exit(EXIT_FAILURE);
	}

	if (soc_version_check(soc_list) == 0) {
		fprintf(stderr, "mxc_vpu_test.out not supported on current soc: %s!\n", strerror(errno));
		exit(EXIT_FAILURE);
	}
	...
	if (using_config_file == 0) {
		...
		strncpy(input_arg[i].cmd.input, "akiyo.mp4", MAX_PATH);
...
int decoder_close(struct decode *dec)
{
	RetCode ret;
...
decode_test(void *arg)
{
	...
	/* start decoding */
	if (decoder_start(dec) < 0) {
		fprintf(stderr, "The decoder_start() function has failed: %s!\n", strerror(errno));
		exit(EXIT_FAILURE);
	}

err1:
	ret = decoder_close(dec);
